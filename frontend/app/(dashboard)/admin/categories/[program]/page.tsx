...
export default function CategoryProgramPage() {
  ...
  const fetchManuscripts = useCallback(async () => {
    ...
    try {
      ...
      params.set("q[program_or_track_eq]", program);
      if (debouncedSearch.trim()) {
        params.set("q[title_or_authors_or_research_type_cont]", debouncedSearch.trim());
      }

      const res = await apiFetch(`${API_BASE_URL}/api/v1/admins/manuscripts?${params.toString()}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  }, [token, page, debouncedSearch, program]);
  ...
  return (
    <div className="flex flex-col gap-6 px-8 py-8">
      <nav className="flex items-center gap-1.5 text-sm text-gray-500" aria-label="Breadcrumb">
        <Link href="/admin/categories" className="hover:text-gray-900 hover:underline underline-offset-2 transition-colors">
          Categories
        </Link>
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          Manuscripts filed under this program or track.
        </p>
      </div>
      ...
      {!loading && !error && manuscripts.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6">
          {manuscripts.map((m) => (
            <ManuscriptCard key={m.id} manuscript={m} href={`/admin/manuscripts/${m.id}`} />
          ))}
        </div>
      )}
