...
export default function AdminStudentShowPage() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const [studentData, manuscriptsData, drData] = await Promise.all([
        studentRes.json(),
        manuscriptsRes.ok ? manuscriptsRes.json() : { data: [] },
        drRes.ok ? drRes.json() : { data: [] },
      ]);

      setStudent(studentData.data);
      setManuscripts(manuscriptsData.data ?? []);
      setDownloadRequests(drData.data ?? []);
    } catch {
  ...
  const avatarSrc = student.avatar_url ? `${API_BASE_URL}${student.avatar_url}` : '/avatar_placeholder.png';
  ...
  return (
    <div className="flex flex-col gap-6 px-8 py-8">
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-8">
        <h2 className="mb-4 text-base font-bold text-gray-900">
          Download Requests{" "}
          <span className="text-sm font-normal text-gray-400">({downloadRequests.length})</span>
        </h2>
        {downloadRequests.length === 0 ? (
          <p className="text-sm text-gray-400">No download requests found.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              <thead className="border-b border-gray-200 bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left font-semibold text-gray-600">Requested On</th>
                  <th className="px-4 py-3 text-left font-semibold text-gray-600">Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {downloadRequests.map((dr) => (
                  <tr key={dr.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3">
                      <Link href={`/admin/manuscripts/${dr.manuscript_id}`} className="font-medium text-primary hover:underline">
                        {dr.manuscript_title ?? `Manuscript #${dr.manuscript_id}`}
                      </Link>
                    </td>
                    <td className="px-4 py-3 text-gray-700">
                      {new Date(dr.created_at).toLocaleDateString(undefined, {
                        year: "numeric",
                        month: "short",
                        day: "numeric",
                      })}
